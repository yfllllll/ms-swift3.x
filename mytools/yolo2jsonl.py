...
def process_dataset(root, dataset_name, train_ratio=0.8):
    
    dataset_dir = os.path.join(root, dataset_name)  # 替换为你的数据集实际目录路径
    ...
    for subfolder, exists in subfolder_structure.items():
        if exists:
            ...
            output_jsonl_path = os.path.join(dataset_dir, f"{subfolder}.jsonl")  # 输出文件名，例如 train.jsonl, val.jsonl
    ...
    if not any(subfolder_structure.values()):
        ...
        image_label_pairs = get_image_label_pairs(image_dir, label_dir)
        train_pairs, val_pairs = split_data(image_label_pairs, train_ratio=train_ratio)
        train_output_path = os.path.join(dataset_dir, "train.jsonl")
        val_output_path = os.path.join(dataset_dir, "val.jsonl")
        write_to_jsonl(train_pairs, classes, train_output_path)
        write_to_jsonl(val_pairs, classes, val_output_path)
...
if __name__ == "__main__":
    # 根目录
    root = "/data1/lyf/formate_t/coco_format"
